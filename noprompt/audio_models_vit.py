...
class VisionTransformer(timm.models.vision_transformer.VisionTransformer):
    ...
    """
    def __init__(self, global_pool=False, mask_2d=True, use_custom_patch=False, n_progr=0, n_seq=256, **kwargs):
        ...
        if self.n_progr > 0:
            self.learnable_prompts_init = nn.Parameter(
                torch.randn(n_progr * (len(self.blocks)//6), 768) * 768**-0.5
            )
            self.learnable_prompts_progr = nn.ParameterList([
                nn.Parameter(torch.randn(n_progr, 768) * 768**-0.5) 
    ...
    def forward_block_post(self, ii, x, x_t):
        ...
        if self.n_progr > 0 and self.learnable_prompts_progr is not None:
            if ii % 6 == 0:
                x[:, self.n_seq+1 + ii//6*self.n_progr : self.n_seq+1 + (ii//6+1)*self.n_progr, :] += \
                    self.learnable_prompts_progr[ii//6]
    ...
    def forward_features(self, x):
        ...
        for ii, blk in enumerate(self.blocks):
            ...
            if self.n_progr > 0 and self.learnable_prompts_progr is not None and ii != 11:
                x[:, 197 + ii*self.n_progr : 197 + (ii+1)*self.n_progr, :] += \
                    self.learnable_prompts_progr[ii]
