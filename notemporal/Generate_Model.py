...
class GenerateModel(nn.Module):
    ...
    def _build_image_model(self, model_name='vit_base_patch16', ckpt_path='./mae_face_pretrain_vit_base.pth',
        ...
        new_posemb = resize_pos_embed(orig_pos_embed, self.image_encoder.pos_embed)
    ...
    def forward(self, image, audio):
        ...
        if t != 16:
            print(f"错误：t的值为{t}（预期16），对应的image形状为{image.shape}", flush=True)
        ...
        try:
            ...
            video_features = torch.mean(image, dim=1)
            ...
            return torch.zeros((n, 1), dtype=torch.float32, device=image.device)
